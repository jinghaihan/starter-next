...
import { findCheckoutPlanByPriceId, resolveCheckoutPlan } from '../../packages/payment/src/catalog'
...
describe('payment catalog', () => {
  ...
  it('finds checkout plan by price id', () => {
    vi.stubEnv('STRIPE_PRICE_PRO_MONTHLY', 'price_monthly_123')

    expect(findCheckoutPlanByPriceId('price_monthly_123')).toEqual({
      key: 'pro_monthly',
      priceId: 'price_monthly_123',
      mode: 'subscription',
      category: 'subscription',
  ...
  it('returns null for unknown price id', () => {
    expect(findCheckoutPlanByPriceId('price_missing')).toBe(null)
  })
})
